...
(function(){
  ...
  function createAuthModal(mode){ // mode: 'login' | 'register' | 'profile'
    ...
    function renderLogin(){
      modal.innerHTML = `
        <div class="modal-header"><h2>Connexion</h2><button class="close-btn" id="authClose">×</button></div>
        <div class="modal-body">
          <label>Email</label><input id="authEmail" type="text" value="admin@asso.cd">
          <label>Mot de passe</label><input id="authPassword" type="password">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px">
            <div style="font-size:13px;color:var(--muted)"><a href="#" id="toRegister">Créer un compte</a></div>
            <div style="display:flex;gap:8px"><button class="btn-ghost" id="authCancel">Annuler</button><button class="btn-primary" id="authSubmit">Se connecter</button></div>
          </div>
    ...
    function renderRegister(){
      modal.innerHTML = `
        <div class="modal-header"><h2>Créer un compte</h2><button class="close-btn" id="authClose">×</button></div>
        <div class="modal-body">
          <label>Nom</label><input id="regName" type="text" placeholder="Votre nom">
          <label>Email</label><input id="regEmail" type="text">
          <label>Mot de passe</label><input id="regPassword" type="password">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px">
            <div style="font-size:13px;color:var(--muted)"><a href="#" id="toLogin">J'ai déjà un compte</a></div>
            <div style="display:flex;gap:8px"><button class="btn-ghost" id="authCancel">Annuler</button><button class="btn-primary" id="regSubmit">Créer</button></div>
          </div>
          <div id="authMsg" style="min-height:18px;margin-top:8px;color:var(--danger)"></div>
        </div>
      `;
