...
async function loadAllAdmin(){
  ...
  try{
    ...
    if(window.api && typeof window.api.getRetraits === 'function'){
      try{ retraits = await window.api.getRetraits(); }catch(e){ retraits = loadRetraits(); }
    } else { retraits = loadRetraits(); }
  ...
  retraits.forEach(r => { if(r.memberCode) balanceMap[r.memberCode] = (balanceMap[r.memberCode]||0) - (parseFloat(r.montant)||0); });
  ...
  retraits.forEach(r => ops.push({ id: r.id || r.createdAt || Date.now(), memberId: r.memberCode, type: 'retrait', amount: Number(r.montant||0), time: new Date(r.createdAt || r.date || Date.now()).getTime() }));
  ...
  if(clearBtn) clearBtn.addEventListener('click', async ()=>{
    ...
    try{
      ...
      if(window.useIndexedDB && await window.useIndexedDB()){
        if(window.idbClear) await window.idbClear(); else if(window.getDB){ const db = await window.getDB(); if(db){ const tx = db.transaction('membres','readwrite'); const store = tx.objectStore('membres'); store.clear(); } }
      }

      // Clear localStorage keys (members, depots, retraits, counters, users and auth)
      localStorage.removeItem('membres');
      localStorage.removeItem('depots');
      localStorage.removeItem('retraits');
